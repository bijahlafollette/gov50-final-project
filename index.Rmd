...
#finance <- finance[, !grepl("^\\.\\.\\.[0-9]{2,4}$", colnames(finance))]
finance
#dataset <- dataset[, -c(14:2011)]
#finance <- finance |>
  select(-...14 - ...1005)

finance <-
  mutate(rule_of_40 = if_else(as.numeric(gsub("%", "", Efficiency)) > 40, "Attractive", "na"))
...
finance <- finance |>
  mutate(Market_Cap_Category = if_else(Market_cap >= 1 & Market_cap <= 1000000, "Mid cap", "small"))
